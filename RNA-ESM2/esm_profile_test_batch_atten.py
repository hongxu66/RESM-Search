...
@dataclass
class ProduceConfig:
    seqfile: str = "/mnt/hdd/xhong/yikun/RNA-ESM2/3zd5_AB.fa"
...
def TS_test(cfg: Config) -> None:
    ...
    with torch.no_grad():
        ...
        logits = model(tokens.to(device),repr_layers=[30],return_contacts=True)
        print(logits.keys())
        aaaa = logits['representations'][30]
        print('aaaa',aaaa)
        print(aaaa.shape)
...
#            print(outfea)
            ...
            pairs_value = (100 * predictions[pairs_idx[0],pairs_idx[1]]).astype(np.int8)
    ...
    with torch.no_grad():
        ...
        for item in range(num):
            if item == num - 1:
                ...
                id,seq = batchids[item * batch_size:(item+1)*batch_size], batchseqs[item * batch_size:(item+1)*batch_size]
            # id, seq = item
            print(id, seq)
            # tokens = torch.from_numpy(rna_map_vocab.encode(seq))
            tokens = torch.from_numpy(rna_map_vocab.encode(seq))
