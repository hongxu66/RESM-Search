...
class DataConfig:
    ...
    model_path: str = str(
        current_directory
        / "ckpt/RNA-ESM2-trans-2a100-mappro-KDNY-epoch=06-valid_F1=0.564.ckpt"
    )
    # device: str = "cpu"
    device: str = "cuda:0"
...
def produce_seqprf(cfg: Config) -> None:
        ...
        for item in seqs:
            ...
            if len(item[1]) > maxLength:
                ...
                for j in range(num):
                    id1 = id + "-" + str(j)
                    seq = item[1][
                        j * (cutLength - overlap) : (j + 1) * (cutLength - overlap)
                        + overlap
                    ]
                    mapdict[id1] = item[0]
                    # seqdict[id1] = seq
    ...
    def prf2map(seq, prf, ss=None):
        ...
        for i in range(len(seq)):
            ...
            if ss == None:
                s += " ".join(
                    ["PRF", str(index), seq[i], "N"]
                    + [str(prf[j, i]) for j in index1]
                    + ["\n"]
                )
            else:
                if ss[i] == ".":
    ...
    seqmapdict, seqs = splitseq(allseqs)
    # print(seqs)
    ...
    with torch.no_grad():
        for item in seqs:
            ...
            tokens = tokens.unsqueeze(0)
            # logits = model(tokens.to(device))["logits"]
            # prediction = model(tokens.to(device),return_contacts=True)
            prediction = model.slim_inference(tokens.to(device))
            # prediction = model.slim_inference(tokens.to(device))
            ...
            outfea = outfea.cpu()
            outfea = (100 * outfea.numpy()).astype(np.int8)
            datas.append((id, seq, outfea))
            # predictions = model(tokens.to(device),return_contacts=True)["contacts"]
            predictions = prediction["contacts"]
            predictions = predictions[:, :, :]
            predictions = predictions.squeeze(0)
            predictions = predictions.cpu()
            predictions = predictions.numpy()

            pairs_idx = np.array((predictions > 0.01).nonzero(), dtype="int16")
            pairs_idx = pairs_idx[:, pairs_idx[0] < pairs_idx[1]]
            pairs_value = (100 * predictions[pairs_idx[0], pairs_idx[1]]).astype(
                np.int8
            )
            pairs = np.stack([pairs_idx[0], pairs_idx[1], pairs_value], axis=-1)
            # conts.append((id,seq,predictions))
            # conts.append((id,seq,pairs))
            conts.append((id, pairs))
            attentions = prediction["attentions"]
            attentions = attentions.squeeze(0)
            attentions = attentions.cpu()
            attentions = attentions.numpy()
            print(attentions.shape)
            attens.append((id, attentions))
    ...
    np.savez("./" + outname + "_attens.npz", a=attens)
    # temout = open('./' + outname + '_allmaps.bpmap','w+')
    # temout.write(allmaps)
    # temout.close()


@hydra.main(config_name="config")
def produce_seqprf_batch(cfg: Config) -> None:
    def writejson(data, outfile):
        with open(outfile, "w") as f:
            json.dump(data, f)
    ...
    allmaps = ""
    batch_size = 8
    batchids, batchseqs = [i[0] for i in seqs], [i[1] for i in seqs]
    num = math.ceil(len(seqs) / batch_size)
    index = 0
    with torch.no_grad():
        # datas = []
        # conts = []
        for item in range(num):
            if item == num - 1:
                id, seq = batchids[item * batch_size :], batchseqs[item * batch_size :]
            else:
                id, seq = (
                    batchids[item * batch_size : (item + 1) * batch_size],
                    batchseqs[item * batch_size : (item + 1) * batch_size],
                )
            # id, seq = item
            print(id, seq)
            # tokens = torch.from_numpy(rna_map_vocab.encode(seq))
            ...
            np.savez("./benchmark_features" + str(index) + ".npz", a=outfea)
            # np.savez('./' + outname + '_features.npz',a=datas)
            # np.savez('./benchmark_attentions.npz',a=attens)
            np.savez("./benchmark_contacts" + str(index) + ".npz", a=predictions)
            index += 1
    # np.savez('./' + outname + '_contacts.npz',a=conts)
    # temout = open('./' + outname + '_allmaps.bpmap', 'w+')
    # temout.write(allmaps)
...
def produce_seqprf_rfam(cfg: Config) -> None:
    ...
    def readpathother(dirpath):
        ...
        for item in dirlists:
            if os.path.exists(
                "/lustre/home/xhong/PostDotWorks/2023/BPmap_new_training/LM/"
                + item
                + "_contacts.npz"
            ):
                continue
