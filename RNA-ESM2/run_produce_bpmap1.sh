...
do
    id=${seqfile##*/}
    if [ -f /mnt/remote_home/xhong/PostWork/2024/RNA-ESM2/results_650_con/${input}_${id}_allmaps.bpmap ];
    then
    # echo CUDA_VISIBLE_DEVICES=1 python esm2embatt_650M_con.py produce.seqfile=${seqfile} produce.outname=chr1_${id}
        CUDA_VISIBLE_DEVICES=1 python esm2embatt_650M_con.py produce.seqfile=${seqfile} produce.outname=${input}_${id}
