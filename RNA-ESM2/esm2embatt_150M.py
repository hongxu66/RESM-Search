...
def produce_seqprf(cfg: Config) -> None:
    ...
    seqs = readfasta1(cfg.produce.seqfile)
#    seq = "GGGGACUCCAGAGGUCGAGAGACCGGAGAUAUCACCC"
#    tokens = torch.from_numpy(vocab.encode(seq))
    ...
    with torch.no_grad():
        for item in seqs:
            ...
            tokens = tokens.unsqueeze(0)
            logits = model(tokens.to(device))["logits"]
            outs = model(tokens.to(device),repr_layers=[30],return_contacts=True)
            start_idx = int(rna_map_vocab.prepend_bos) ##
            end_idx = logits.size(-2) - int(rna_map_vocab.append_eos) ##
            emb = outs['representations'][30][:,start_idx:end_idx,:]
            emb = emb.squeeze()
            try:
                emb = F.softmax(emb,dim=1)
            except IndexError:
                pass
            att = outs['attentions'].squeeze(0)[:,:,start_idx:end_idx,start_idx:end_idx].permute(2,3,0,1)
            embedding_path = os.path.join(results_path,id + '_emb.npy')
            attentions_path = os.path.join(results_path,id + '_atp.npy')
            np.save(embedding_path,emb.cpu().numpy())
            np.save(attentions_path,att.cpu().numpy())
#             print(logits.shape)
